$(document).ready(function () {
	...
	//判断显示导航栏图标
	var path = $("#path").val();
	if (path == "set_info.php") {
		$('li:eq(15)').css("border-bottom","2px solid gray");
	} 
	else if (path == "set_avatar.php") {
		$('li:eq(16)').css("border-bottom","2px solid gray");
	} 
	else{
		$('li:eq(17)').css("border-bottom","2px solid gray");
	};
	//密码修改认证
	...
	var state2 = false;
		...
		if ($("#newPassword").val() == '') {
			...
			if ($("#newPassword").val().length < 6 || $("#newPassword").val().length > 20) {
				...
				var empty = false;
				for (var i = $("#newPassword").val().length - 1; i >= 0; i--) {
					if ($("#newPassword").val().substr(i,1)==" ") {
						$("i.pwtip").css({"display":"none"});
						$("span.pwtip").html("密码不能含有空格");
						$("#newPassword").val('');
						empty = true;
					};
				};
