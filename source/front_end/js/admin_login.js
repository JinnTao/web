window.onload = function(){
	document.getElementById("check").onclick = to_check_adname_and_pwd;
...
function to_check_adname_and_pwd(){
	var o = createXMLHttpRequest();//new an ajax object
	var name = document.getElementById("name").value;
	var pwd = document.getElementById("pwd").value;
	var url	= "index.php?rt=admin/login_manager&adname=" + name + "&adpwd=" + pwd;
	...
	o.onreadystatechange = function(){
		if (o.readyState == 4){ //only the readystate is 4 can we get the value of xmlHttp.status
			if (o.status == 200){
				...
				if (check){
					window.location.href = "index.php?rt=admin/manage";
				}
				else{
